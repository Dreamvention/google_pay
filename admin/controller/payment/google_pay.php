...
class ControllerPaymentGooglePay extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/google_pay', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('payment/google_pay', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (VERSION >= '2.2.0.0') {
			$this->response->setOutput($this->load->view('payment/google_pay', $data));
		} else {
			$this->response->setOutput($this->load->view('payment/google_pay.tpl', $data));
		}
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'payment/google_pay')) {
